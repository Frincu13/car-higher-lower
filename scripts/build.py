...
# Curated roster: the game cars listed in scripts/keep.csv (fun, well-known ones) plus
# hand-picked extras from scripts/extra_cars.csv (matched by match_extra.py).
keep = {tuple(r.split(';')) for r in open(os.path.join(HERE, 'keep.csv'), encoding='utf-8').read().splitlines()[1:]}
kept = [c for c in kept if (c['name'], c['years']) in keep]
# Obvious unit errors in the source (pounds stored as kg).
WEIGHT_FIX = {('Toyota FJ Cruiser', '2007'): 1950}
extra_path = os.path.join(LISTS, 'extra_matched.json')
for m in (json.load(open(extra_path, encoding='utf-8')) if os.path.exists(extra_path) else []):
    name, year = m['name'], m['year']
    v = {f: m[f] for f in F}
    v['weight'] = WEIGHT_FIX.get((name, str(year)), v['weight'])
    for f, (lo, hi) in RANGE.items():
        if v[f] is not None and not lo <= v[f] <= hi:
            log['range'].append(f'{name} {year}: {f}={v[f]}'); v[f] = None
    engine = clean_engine(m['engine'])
    for f in LEAK_FIELDS:
        if v[f] and leaks(v[f], f'{name} {engine}'):
            log['leak'].append(f'{name}: {f}'); v[f] = None
    rnd = lambda x, n=0: None if x is None else (round(x, n) if n else int(round(x)))
    img = images.get(f'{name}|{year}')
    kept.append({
        'name': name, 'years': str(year), 'engine': engine, 'games': [],
        'hp': rnd(v['hp']), 'torque': rnd(v['torque']), 'accel': rnd(v['accel'], 1),
        **({'accelEst': rnd(estimate_0_100(v['hp'], v['weight']), 1)} if v['accel'] is None and v['hp'] and v['weight'] else {}),
        'topSpeed': rnd(v['topSpeed']), 'weight': rnd(v['weight']),
        **({'image': img['thumb'], 'credit': img['credit'], 'license': img['license'], 'source': img['page']} if img else {}),
    })
# A 0-100 time far slower than power-to-weight allows is a data error (Hummer H2: 20 s).
for c in kept:
    if c['accel'] and c['hp'] and c['weight'] and c['accel'] > 2 * estimate_0_100(c['hp'], c['weight']):
        log['range'].append(f"{c['name']} {c['years']}: accel={c['accel']} implausibly slow"); c['accel'] = None
# Every roster car, photo or not: fetch_images.py looks photos up for this list.
json.dump([{'name': c['name'], 'years': c['years']} for c in kept],
          open(os.path.join(LISTS, 'candidates.json'), 'w', encoding='utf-8'), ensure_ascii=False)
...
print('draft pool (hp, torque, weight, grades)', sum(1 for c in kept if c.get('grades') and c['hp'] and c['torque'] and c['weight']))
