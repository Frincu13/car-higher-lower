...
# How well known each car is to a general audience: 3 = everyone knows it, 2 = car fans,
# 1 = niche. The games show 3 and 2 by default (3 more often); "Toate" adds the rest.
fame = {}
for row in open(os.path.join(HERE, 'fame.csv'), encoding='utf-8').read().splitlines()[1:]:
    fname, fyears, f = row.split(';')
    fame[(fname, fyears)] = int(f)
for c in kept:
    c['fame'] = fame.pop((c['name'], c['years']), 2)
if fame:
    print('fame.csv rows with no matching car:', sorted(fame))
