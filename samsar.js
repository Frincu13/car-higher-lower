...
(() => {
  ...
  // vânătoarea, nu notele. O mașină care o încalcă pierde un punct din notă.
  const REGULI = [
    'doar mașini cu cutie manuală',
    'doar mașini mai vechi de 15 ani',
    'doar mașini sub 150.000 km',
    'doar break, combi sau familială',
    'doar diesel',
    'doar mașini care nu sunt germane',
    'doar mașini cu tracțiune spate',
    'doar anunțuri din afara României',
    'doar mașini cu o singură culoare de caroserie, nu gri, nu negru, nu alb',
    'doar mașini cu mai mult de doi proprietari',
  ];
  ...
  const bani = n => `${new Intl.NumberFormat('ro-RO').format(n)} €`;
  ...
  function trage() {
    ...
    while (surprize.length < 2) {
      const clar = pachet.splice(Math.floor(Math.random() * pachet.length), 1)[0];
    ...
    state.runda = {
      ...
      maxim: sursa.maxim,
      surprize,
      regula: state.mod === 'mix' && Math.random() < 0.55 ? rnd(REGULI) : null,
    };
    state.rezultat = null;
    state.linkuri = ['', ''];
    tine();
    ...
    $('sms-draw').innerHTML = r.mod === 'client' ? `
      <article class="sms-card">
        <div class="sms-card-main">
          ...
          <p class="sms-card-job">${esc(c.ocupatie)}</p>
          <p class="sms-card-ctx">${esc(c.context)}</p>
          <ul class="sms-uz">${c.uz.map(u => `<li>${esc(u)}</li>`).join('')}</ul>
          <p class="sms-card-vrea">${esc(c.vrea)}</p>
        </div>
        <div class="sms-card-side">
          <div class="sms-budget">
            <span class="sms-b-k">Buget</span>
            <strong>${bani(c.buget[0])} - ${bani(c.buget[1])}</strong>
            <span class="sms-b-max">maxim absolut ${bani(c.maxim)}</span>
          </div>
      ...
      <article class="sms-card sms-card-cat">
        <div class="sms-card-main">
          ...
          <h3 class="sms-card-name">${esc(c.nume)}<em>${esc(c.tag)}</em></h3>
          <p class="sms-card-ctx">${esc(c.desc)}</p>
        </div>
        <div class="sms-card-side">
          <div class="sms-budget">
            <span class="sms-b-k">Buget</span>
  ...
  function randeazaBrief() {
    const r = state.runda, c = r.sursa;
  ...
  function eticheta(k) {
    ...
    if (k === 'surpriza_1') return `surpriza 1: ${r.surprize[0].clar}`;
    if (k === 'surpriza_2') return `surpriza 2: ${r.surprize[1].clar}`;
    return S.label(k);
  }

  function scaraPret() {
    const [a, b] = state.runda.buget, max = state.runda.maxim;
    return [
      `între ${bani(a)} și ${bani(b)} → 10`,
      `peste țintă, până la ${bani(max)} → 7`,
      `peste maxim, până la ${bani(Math.round(max * 1.2))} → 4`,
      `peste atât → 1`,
    ].join('\n');
  }
  ...
  function construiestePrompt() {
    ...
    const cine = r.mod === 'client'
      ? `CLIENTUL
${c.nume}, ${c.varsta} de ani, ${c.ocupatie}.
${c.context}
Cum folosește mașina: ${c.uz.join('; ')}.
Ce vrea: ${c.vrea}
Buget țintă: ${bani(c.buget[0])} - ${bani(c.buget[1])}. Maxim absolut: ${bani(c.maxim)}.`
      : `CATEGORIA
${c.nume}, ${c.tag}.
${c.desc}
Buget țintă: ${bani(c.buget[0])} - ${bani(c.buget[1])}. Maxim absolut: ${bani(c.maxim)}.`;
    ...
    return `Ești arbitru într-un joc care se numește Cel mai bun samsar. Doi jucători au adus câte un anunț real de mașină pentru aceeași cerere. Tu dai note de la 1 la 10, unde 10 e cel mai bun, și spui cine a ales mai bine.

${cine}

${r.mod === 'client' ? 'CRITERIILE CLIENTULUI, în ordinea importanței' : 'CRITERIILE CATEGORIEI, în ordinea importanței'}
${criterii}

PRICE FIT, scară fixă
${scaraPret()}

CELE DOUĂ SURPRIZE, codate
Fiecare literă a fost mutată cu ${S.ROT} poziții în față în alfabetul englez. Scade ${S.ROT} din fiecare literă ca să afli cuvântul.
surpriza 1: ${r.surprize[0].cod}
surpriza 2: ${r.surprize[1].cod}
Decodează-le întâi, apoi dă fiecărei mașini o notă pentru cât de bine stă la surpriza aia.
${r.regula ? `\nREGULA RUNDEI\n${r.regula}. O mașină care nu o respectă primește o notă mult mai mică la criteriul cel mai important și o spui explicit în verdict.\n` : ''}
ANUNȚURILE
${numeJucator(0)}: ${state.linkuri[0]}
${numeJucator(1)}: ${state.linkuri[1]}

CUM LUCREZI
1. Deschide ambele anunțuri. Citește prețul, anul, kilometrajul, motorizarea, dotările și descrierea, inclusiv pozele dacă le poți vedea.
2. Scrie mai întâi faptele extrase din fiecare anunț. Abia după aceea dă note.
3. Notele au o zecimală. Nu da aceeași notă la amândouă decât dacă sunt chiar egale la criteriul ăla.
4. Fiecare notă are un motiv de un rând, care citează ceva concret din anunț. Dacă anunțul nu spune nimic despre criteriu, zi asta în motiv și notează prudent.
5. Totalul nu are voie să fie egal. Dacă iese egal, ajustează la criteriul cel mai puțin important.
6. Nu inventa dotări care nu apar în anunț.

CUM RĂSPUNZI
Întâi un bloc de cod cu JSON exact în forma de mai jos, cu numere, nu text. Apoi, sub bloc, explicațiile pe categorii, în cuvintele tale.
...
"masini" în ordinea jucătorilor, ${numeJucator(0)} prima.
...
"verdict" e o frază care spune cine câștigă și din ce a câștigat.`;
  ...
  function dinTabel(text) {
    const chei = cheiRunda();
    const tinte = chei.map(k => ({ k, n: curata(eticheta(k)) }));
    const scoruri = {};
    for (const linie of text.split('\n')) {
      ...
      if (!nume || /^(suma|categorie|nota)/.test(nume)) continue;
  ...
  function citeste(text) {
    ...
    if (randuri.length < 4) {
      return { eroare: `Am găsit prea puține note (lipsesc: ${lipsa.slice(0, 4).join(', ')}). Cere-i agentului blocul JSON din instrucțiune.` };
    }

    const masini = (obj.masini || []).slice(0, 2);
    ...
    return { randuri, masini, lipsa, gresit, verdict: obj.verdict || '' };
  }
  ...
  function socoteste(randuri) {
    ...
    return {
      suma,
      simpla: [suma[0] / n, suma[1] / n],
      client: [pond[0] / greutate, pond[1] / greutate],
    };
  }
  ...
  function randeazaRezultat(rez) {
    ...
    const masina = i => {
      ...
      const fapte = [m.an, m.km ? `${new Intl.NumberFormat('ro-RO').format(m.km)} km` : '',
        m.cp ? `${m.cp} CP` : '', m.combustibil, m.tractiune].filter(Boolean);
      return `<article class="sms-res ${i === cast ? 'is-win' : ''}">
        ...
        <div class="sms-res-notes">
          <span class="sms-note"><b>${nota2(note.client[i])}</b><em>nota ${r.mod === 'client' ? 'clientului' : 'categoriei'}</em></span>
          <span class="sms-note sms-note-2"><b>${nota2(note.simpla[i])}</b><em>nota simplă</em></span>
        </div>
        ${pret ? `<p class="sms-fit ${pret.s[i] >= 10 ? 'is-in' : pret.s[i] >= 7 ? 'is-over' : 'is-out'}">
          ${pret.s[i] >= 10 ? 'În buget' : pret.s[i] >= 7 ? 'Peste țintă' : 'Peste maxim'} · price fit ${nota1(pret.s[i])}</p>` : ''}
        <a class="sms-res-link" href="${esc(state.linkuri[i])}" target="_blank" rel="noopener">Vezi anunțul</a>
      </article>`;
    };

    const bare = decisive.map(x => {
      const lat = Math.abs(x.d) / maxD * 50;
      const catre = x.d > 0 ? 0 : 1;
      return `<li class="sms-bar ${catre === 0 ? 'to-0' : 'to-1'}">
        <span class="sms-bar-k">${esc(x.eticheta)}</span>
        <span class="sms-bar-track"><i style="width:${lat.toFixed(1)}%"></i></span>
    ...
    $('s-out').innerHTML = `
      <div class="sms-wrap">
        <p class="sms-brief-k">Runda #${r.nr} · ${esc(r.sursa.nume)}</p>
        ...
        <div class="sms-table-wrap">
          ...
          <table class="sms-table">
            ...
            <tfoot>
              ...
              <tr class="is-final"><th scope="row">Nota ${r.mod === 'client' ? 'clientului' : 'categoriei'}</th><td>${nota2(note.client[0])}</td><td>${nota2(note.client[1])}</td></tr>
            </tfoot>
          </table>
        </div>
  ...
  if (pastrata && pastrata.criterii && pastrata.surprize) {
